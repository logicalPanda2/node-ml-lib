...
import { getDims } from "./../models/utils/index.js";

const stats = new Statistics();

console.log(`Suite 0: Validation Functions`);

console.log(`  Group 1: vector validation`);
...
console.log(`  Group 2: matrix validation`);
...
test("throws error when matrix is not a 2D Float64Array", stats, () => {
    try {
        Shared.validateStructureMatSingle(undefined);
...
test("throws error when matrix has no rows", stats, () => {
    try {
        Shared.validateStructureMatSingle(new Array());

        return false;
    } catch(e) {
        return e.message === "Matrix must be a 2D Float64Array";
        // check ln 78 for elaboration on error message
    }
});
test("throws error when matrix has zero-length cols", stats, () => {
    try {
        Shared.validateStructureMatSingle([
            new Float64Array(0),
            new Float64Array(0),
        ]);
...
console.log(`  Group 3: 3D tensor validation`);
test("throws error when one 3D tensor is not a 3D Float64Array", stats, () => {
    try {
        Shared.validateStructure3DTensor(undefined, [[new Float64Array([2, 2]), new Float64Array([2, 2])]]);

        return false;
    } catch(e) {
        return e.message === "Both tensors must be a 3D Float64Array";
    }
});
test("throws error when one 3D tensor has a zero-length dimension", stats, () => {
    try {
        Shared.validateStructure3DTensor([[new Float64Array(0)]], [[new Float64Array([2, 2]), new Float64Array([2, 2])]]);

        return false;
    } catch(e) {
        return e.message === "Both 3D tensors must have positive length in all dimensions";
    }
});
test("throws error when 3D tensor is not a 3D Float64Array", stats, () => {
    try {
        Shared.validateStructure3DTensorSingle(undefined);

        return false;
    } catch(e) {
        return e.message === "Tensor must be a 3D Float64Array";
    }
});
test("throws error when 3D tensor has a zero-length dimension", stats, () => {
    try {
        Shared.validateStructure3DTensorSingle([[new Float64Array(0)]]);

        return false;
    } catch(e) {
        return e.message === "Tensor must have positive length in all dimensions";
    }
});
test("throws error when both 3D tensors' dimensions don't match", stats, () => {
    try {
        Shared.validateSameDimensions3D(
            [
                [
                    new Float64Array(2),
                    new Float64Array(2),
        ...
        return e.message === "Both 3D tensors must have the same dimensions";
    }
});

console.log(`  Group 4: 4D tensor validation`);
test("throws error when one 4D tensor is not a 4D Float64Array", stats, () => {
    try {
        Shared.validateStructureTensor(undefined, [[[new Float64Array([2, 2]), new Float64Array([2, 2])]]]);

        return false;
    } catch(e) {
        return e.message === "Both tensors must be a 4D Float64Array";
    }
});
test("throws error when one 4D tensor has a zero-length dimension", stats, () => {
    try {
        Shared.validateStructureTensor([[[new Float64Array(0)]]], [[[new Float64Array([2, 2]), new Float64Array([2, 2])]]]);

        return false;
    } catch(e) {
        return e.message === "Both 4D tensors must have positive length in all dimensions";
    }
});
test("throws error when 4D tensor is not a 4D Float64Array", stats, () => {
    try {
        Shared.validateStructureTensorSingle(undefined);

        return false;
    } catch(e) {
        return e.message === "Tensor must be a 4D Float64Array";
    }
});
test("throws error when 4D tensor has a zero-length dimension", stats, () => {
    try {
        Shared.validateStructureTensorSingle([[[new Float64Array(0)]]]);
...
test("throws error when both 4D tensors' dimensions don't match", stats, () => {
    try {
        Shared.validateSameDimensions(
            [[
                [
                    new Float64Array(2),
                    new Float64Array(2),
        ...
        return e.message === "Both tensors must have the same dimensions";
    }
});
...
test("divides a matrix by a scalar", stats, () => {
    const m = [
        new Float64Array([7, 4, 1]),
        new Float64Array([2, 2, 9]),
    ];
    const s = 2;
    const expected = [
        [m[0][0] / s, m[0][1] / s],
    ...
    const result = Matrix.divScalar(m, s);
...
test("broadcast-adds a vector to a matrix", stats, () => {
    const m = [
        new Float64Array([6, 1, 9, 0]),
        new Float64Array([7, 3, 4, 2]),
    ];
    const v = new Float64Array([2, 1, 2, 3]);
    const expected = [
        [m[0][0] + v[0], m[0][1] + v[1], m[0][2] + v[2], m[0][3] + v[3]],
        [m[1][0] + v[0], m[1][1] + v[1], m[1][2] + v[2], m[1][3] + v[3]],
    ];
    const result = Matrix.addBroadcast(m, v);
    ...
    try {
        Matrix.addBroadcast(
            [
                ...
                new Float64Array([3, 1])
            ],
            new Float64Array([2, 1, 2])
        );

        return false;
    } catch(e) {
        return e.message === "Vector length must be the same as matrix column length for matrix vector multiplication";
    }
});
...
test("throws error when columns of A is not equal to rows of B", stats, () => {
    try {
        ...
        return e.message === "The columns of A must be equal to the rows of B for matrix multiplication. Try transposing a matrix or flipping the order of arguments";
    }
});
...
console.log(`  Group 5: cross-correlation`);
...
test("cross-correlates a source matrix with a kernel", stats, () => {
    ...
    const result = Matrix.crossCorrelate(src, k);
...
test("performs full mode cross-correlation", stats, () => {
    ...
    const result = Matrix.fullCrossCorrelate(src, k);
...
test("computes the swish derivative of a tensor", stats, () => {
    ...
    const a = [
        [
            new Float64Array([t[0][0][0] * (1 / (1 + Math.exp(-t[0][0][0]))), t[0][0][1] * (1 / (1 + Math.exp(-t[0][0][1])))]),
            new Float64Array([t[0][1][0] * (1 / (1 + Math.exp(-t[0][1][0]))), t[0][1][1] * (1 / (1 + Math.exp(-t[0][1][1])))]),
        ],
        [
            new Float64Array([t[1][0][0] * (1 / (1 + Math.exp(-t[1][0][0]))), t[1][0][1] * (1 / (1 + Math.exp(-t[1][0][1])))]),
            new Float64Array([t[1][1][0] * (1 / (1 + Math.exp(-t[1][1][0]))), t[1][1][1] * (1 / (1 + Math.exp(-t[1][1][1])))]),
        ],
        [
            new Float64Array([t[2][0][0] * (1 / (1 + Math.exp(-t[2][0][0]))), t[2][0][1] * (1 / (1 + Math.exp(-t[2][0][1])))]),
            new Float64Array([t[2][1][0] * (1 / (1 + Math.exp(-t[2][1][0]))), t[2][1][1] * (1 / (1 + Math.exp(-t[2][1][1])))]),
        ],
    ];
    const expected = [
        [
...
test("completes a backward pass to a previous pooled layer", stats, () => {
    ...
    const b1 = Vector.create(3, -0.05, 0.05);
    const W2 = Array.from(
        { length: 4 },
        () => Array.from(
            { length: 3 },
    ...
    const b2 = Vector.create(4, -0.05, 0.05);
    ...
    const expected = [
        Matrix.add(
            Matrix.fullCrossCorrelate(z2[0], Matrix.flip(W2[0][0])),
            Matrix.add(
                Matrix.fullCrossCorrelate(z2[1], Matrix.flip(W2[1][0])),
                Matrix.add(
                    Matrix.fullCrossCorrelate(z2[2], Matrix.flip(W2[2][0])),
                    Matrix.fullCrossCorrelate(z2[3], Matrix.flip(W2[3][0])),
                ),
            ),
        ),
        Matrix.add(
            Matrix.fullCrossCorrelate(z2[0], Matrix.flip(W2[0][1])),
            Matrix.add(
                Matrix.fullCrossCorrelate(z2[1], Matrix.flip(W2[1][1])),
                Matrix.add(
                    Matrix.fullCrossCorrelate(z2[2], Matrix.flip(W2[2][1])),
                    Matrix.fullCrossCorrelate(z2[3], Matrix.flip(W2[3][1])),
                ),
            ),
        ),
        Matrix.add(
            Matrix.fullCrossCorrelate(z2[0], Matrix.flip(W2[0][2])),
            Matrix.add(
                Matrix.fullCrossCorrelate(z2[1], Matrix.flip(W2[1][2])),
                Matrix.add(
                    Matrix.fullCrossCorrelate(z2[2], Matrix.flip(W2[2][2])),
                    Matrix.fullCrossCorrelate(z2[3], Matrix.flip(W2[3][2])),
                ),
            ),
        ),
    ];
    const result = NeuralNet.convBackwardPrev(W2, z2);
    // z2 has the same shape as the gradient wrt. z2, so the result has valid shape
    // note that this is just a shortcut, in actual models z2 would be replaced with dL/dz2

    // floating point error is weirdly prevalent here so truncate to 6 decimal digits
    return (
        Number(expected[0][0][0].toFixed(6)) === Number(result[0][0][0].toFixed(6)) &&
        Number(expected[0][0][1].toFixed(6)) === Number(result[0][0][1].toFixed(6)) &&
        Number(expected[0][0][2].toFixed(6)) === Number(result[0][0][2].toFixed(6)) &&
        Number(expected[0][0][3].toFixed(6)) === Number(result[0][0][3].toFixed(6)) &&
        Number(expected[0][1][0].toFixed(6)) === Number(result[0][1][0].toFixed(6)) &&
        Number(expected[0][1][1].toFixed(6)) === Number(result[0][1][1].toFixed(6)) &&
        Number(expected[0][1][2].toFixed(6)) === Number(result[0][1][2].toFixed(6)) &&
        Number(expected[0][1][3].toFixed(6)) === Number(result[0][1][3].toFixed(6)) &&
        Number(expected[0][2][0].toFixed(6)) === Number(result[0][2][0].toFixed(6)) &&
        Number(expected[0][2][1].toFixed(6)) === Number(result[0][2][1].toFixed(6)) &&
        Number(expected[0][2][2].toFixed(6)) === Number(result[0][2][2].toFixed(6)) &&
        Number(expected[0][2][3].toFixed(6)) === Number(result[0][2][3].toFixed(6)) &&
        Number(expected[0][3][0].toFixed(6)) === Number(result[0][3][0].toFixed(6)) &&
        Number(expected[0][3][1].toFixed(6)) === Number(result[0][3][1].toFixed(6)) &&
        Number(expected[0][3][2].toFixed(6)) === Number(result[0][3][2].toFixed(6)) &&
        Number(expected[0][3][3].toFixed(6)) === Number(result[0][3][3].toFixed(6)) &&
        Number(expected[1][0][0].toFixed(6)) === Number(result[1][0][0].toFixed(6)) &&
        Number(expected[1][0][1].toFixed(6)) === Number(result[1][0][1].toFixed(6)) &&
        Number(expected[1][0][2].toFixed(6)) === Number(result[1][0][2].toFixed(6)) &&
        Number(expected[1][0][3].toFixed(6)) === Number(result[1][0][3].toFixed(6)) &&
        Number(expected[1][1][0].toFixed(6)) === Number(result[1][1][0].toFixed(6)) &&
        Number(expected[1][1][1].toFixed(6)) === Number(result[1][1][1].toFixed(6)) &&
        Number(expected[1][1][2].toFixed(6)) === Number(result[1][1][2].toFixed(6)) &&
        Number(expected[1][1][3].toFixed(6)) === Number(result[1][1][3].toFixed(6)) &&
        Number(expected[1][2][0].toFixed(6)) === Number(result[1][2][0].toFixed(6)) &&
        Number(expected[1][2][1].toFixed(6)) === Number(result[1][2][1].toFixed(6)) &&
        Number(expected[1][2][2].toFixed(6)) === Number(result[1][2][2].toFixed(6)) &&
        Number(expected[1][2][3].toFixed(6)) === Number(result[1][2][3].toFixed(6)) &&
        Number(expected[1][3][0].toFixed(6)) === Number(result[1][3][0].toFixed(6)) &&
        Number(expected[1][3][1].toFixed(6)) === Number(result[1][3][1].toFixed(6)) &&
        Number(expected[1][3][2].toFixed(6)) === Number(result[1][3][2].toFixed(6)) &&
        Number(expected[1][3][3].toFixed(6)) === Number(result[1][3][3].toFixed(6)) &&
        Number(expected[2][0][0].toFixed(6)) === Number(result[2][0][0].toFixed(6)) &&
        Number(expected[2][0][1].toFixed(6)) === Number(result[2][0][1].toFixed(6)) &&
        Number(expected[2][0][2].toFixed(6)) === Number(result[2][0][2].toFixed(6)) &&
        Number(expected[2][0][3].toFixed(6)) === Number(result[2][0][3].toFixed(6)) &&
        Number(expected[2][1][0].toFixed(6)) === Number(result[2][1][0].toFixed(6)) &&
        Number(expected[2][1][1].toFixed(6)) === Number(result[2][1][1].toFixed(6)) &&
        Number(expected[2][1][2].toFixed(6)) === Number(result[2][1][2].toFixed(6)) &&
        Number(expected[2][1][3].toFixed(6)) === Number(result[2][1][3].toFixed(6)) &&
        Number(expected[2][2][0].toFixed(6)) === Number(result[2][2][0].toFixed(6)) &&
        Number(expected[2][2][1].toFixed(6)) === Number(result[2][2][1].toFixed(6)) &&
        Number(expected[2][2][2].toFixed(6)) === Number(result[2][2][2].toFixed(6)) &&
        Number(expected[2][2][3].toFixed(6)) === Number(result[2][2][3].toFixed(6)) &&
        Number(expected[2][3][0].toFixed(6)) === Number(result[2][3][0].toFixed(6)) &&
        Number(expected[2][3][1].toFixed(6)) === Number(result[2][3][1].toFixed(6)) &&
        Number(expected[2][3][2].toFixed(6)) === Number(result[2][3][2].toFixed(6)) &&
        Number(expected[2][3][3].toFixed(6)) === Number(result[2][3][3].toFixed(6))
    );
});
test("completes an un-pooling backward pass", stats, () => {
    ...
    // again, z2 is a shortcut
    const expected = Array.from(
        { length: z1.length },
        () => Array.from(
            { length: z1[0].length },
            () => new Float64Array(z1[0][0].length)
        )
    );
    for(let k = 0; k < z1.length; k++) {
        for(let i = 0; i + 1 < z1[0].length; i += 2) {
            for(let j = 0; j + 1 < z1[0][0].length; j += 2) {
                switch(z1_pooled[k][i / 2][j / 2]) {
                    case z1[k][i][j]:
                        expected[k][i][j] = dl_dz1_pooled[k][i / 2][j / 2] * mockActivations[k][i][j];
                    break;
                    case z1[k][i][j + 1]:
                        expected[k][i][j + 1] = dl_dz1_pooled[k][i / 2][j / 2] * mockActivations[k][i][j + 1];
                    break;
                    case z1[k][i + 1][j]:
                        expected[k][i + 1][j] = dl_dz1_pooled[k][i / 2][j / 2] * mockActivations[k][i + 1][j];
                    break;
                    case z1[k][i + 1][j + 1]:
                        expected[k][i + 1][j + 1] = dl_dz1_pooled[k][i / 2][j / 2] * mockActivations[k][i + 1][j + 1];
                    break;
                }
            }
        }
    }
    const result = NeuralNet.convBackward(z1, z1_pooled, dl_dz1_pooled, mockActivations);

    for(let i = 0; i < expected.length; i++) {
        for(let j = 0; j < expected[0].length; j++) {
            for(let k = 0; k < expected[0][0].length; k++) {
                if(expected[i][j][k] !== result[i][j][k]) return false;
            }
        }
    }
...
test("completes a backward pass from a dense layer to a conv layer with GAP", stats, () => {
    const mockRgbImage = Array.from(
        { length: 3 }, 
        () => Matrix.create(10, 10, -0.05, 0.05)
    );
    ...
    const result = NeuralNet.convBackwardGAP(
        b3_grad, W3, 5, 2, 2,
        Activations.swishDerivativeT(a2, z2),
    );
    const delta_l = Matrix.matrixVector(Matrix.transpose(W3), b3_grad);
    const activationGrad_l = Activations.swishDerivativeT(a2, z2);
    const expected = new Array(5);
    for(let i = 0; i < delta_l.length; i++) {
        expected[i] = new Array(2);
        const grad = delta_l[i] / (2 * 2);
        for(let j = 0; j < 2; j++) {
            expected[i][j] = new Float64Array(2)
            for(let k = 0; k < 2; k++) {
                expected[i][j][k] = grad * activationGrad_l[i][j][k];
            }
        }
    }
...
console.log(`  Group 3: Transformers`);
test("successfully initializes the sinusoidal matrix for postional encoding", stats, () => {
    const max_seq = 3;
    const d_model = 3;
    const expected = [
        new Float64Array([
            Math.sin( 0 / (Math.pow(10000, ((2 * 0) / d_model)))),
            Math.cos( 0 / (Math.pow(10000, ((2 * 0) / d_model)))),
            Math.sin( 0 / (Math.pow(10000, ((2 * 1) / d_model)))),
        ]),
        new Float64Array([
            Math.sin( 1 / (Math.pow(10000, ((2 * 0) / d_model)))),
            Math.cos( 1 / (Math.pow(10000, ((2 * 0) / d_model)))),
            Math.sin( 1 / (Math.pow(10000, ((2 * 1) / d_model)))),
        ]),
        new Float64Array([
            Math.sin( 2 / (Math.pow(10000, ((2 * 0) / d_model)))),
            Math.cos( 2 / (Math.pow(10000, ((2 * 0) / d_model)))),
            Math.sin( 2 / (Math.pow(10000, ((2 * 1) / d_model)))),
        ]),
    ];
    const result = NeuralNet.initSinusoidal(max_seq, d_model);
...
test("successfully transforms a raw string into embeddings", stats, () => {
    const string = "hello , i have arrived .";
    const vocab = {
        "hello": 0,
        ",": 1,
        "i": 2,
    ...
    const position = NeuralNet.initSinusoidal(20, d_model);
    const expected = [
        Vector.add(E[6], position[0]),
        Vector.add(E[0], position[1]),
        Vector.add(E[1], position[2]),
        Vector.add(E[2], position[3]),
        Vector.add(E[3], position[4]),
        Vector.add(E[4], position[5]),
        Vector.add(E[5], position[6]),
    ];
    const result = NeuralNet.tokenize(string, vocab, E, position);
...
test("successfully runs the attention mechanism on an embedded sentence", stats, () => {
    const seq_len = 3;
    const d_model = 3;
    const mockEmbedded = Matrix.create(seq_len, d_model, -0.05, 0.05);
    ...
    const Q = Matrix.multiply(mockEmbedded, Wq);
    const K = Matrix.multiply(mockEmbedded, Wk);
    const V = Matrix.multiply(mockEmbedded, Wv);
    const QK_T = Matrix.divScalar(
        Matrix.multiply(Q, Matrix.transpose(K)),
        d_model
    );
    const A = QK_T.map(vector => Activations.softmax(vector));
    const expected = Matrix.multiply(A, V);

    const [result, _] = NeuralNet.attention(mockEmbedded, Wq, Wk, Wv, d_model);
...
test("completes layer-normalization on a matrix", stats, () => {
    const mat = Matrix.create(4, 3, -0.05, 0.05);
    const scale = Vector.create(3, -0.05, 0.05);
    const shift = Vector.create(3, -0.05, 0.05);
    const means = [
        (mat[0][0] + mat[0][1] + mat[0][2]) / 3,
        (mat[1][0] + mat[1][1] + mat[1][2]) / 3,
        (mat[2][0] + mat[2][1] + mat[2][2]) / 3,
        (mat[3][0] + mat[3][1] + mat[3][2]) / 3,
    ];
    const vars = [
        (Math.pow(mat[0][0] - means[0], 2) + Math.pow(mat[0][1] - means[0], 2) + Math.pow(mat[0][2] - means[0], 2)) / 3,
        (Math.pow(mat[1][0] - means[1], 2) + Math.pow(mat[1][1] - means[1], 2) + Math.pow(mat[1][2] - means[1], 2)) / 3,
        (Math.pow(mat[2][0] - means[2], 2) + Math.pow(mat[2][1] - means[2], 2) + Math.pow(mat[2][2] - means[2], 2)) / 3,
        (Math.pow(mat[3][0] - means[3], 2) + Math.pow(mat[3][1] - means[3], 2) + Math.pow(mat[3][2] - means[3], 2)) / 3,
    ]
    const expected = [
        [
            scale[0] * ((mat[0][0] - means[0]) / Math.sqrt(vars[0] + 1e-8)) + shift[0],
            scale[1] * ((mat[0][1] - means[0]) / Math.sqrt(vars[0] + 1e-8)) + shift[1],
            scale[2] * ((mat[0][2] - means[0]) / Math.sqrt(vars[0] + 1e-8)) + shift[2],
        ],
        [
            scale[0] * ((mat[1][0] - means[1]) / Math.sqrt(vars[1] + 1e-8)) + shift[0],
            scale[1] * ((mat[1][1] - means[1]) / Math.sqrt(vars[1] + 1e-8)) + shift[1],
            scale[2] * ((mat[1][2] - means[1]) / Math.sqrt(vars[1] + 1e-8)) + shift[2],
        ],
        [
            scale[0] * ((mat[2][0] - means[2]) / Math.sqrt(vars[2] + 1e-8)) + shift[0],
            scale[1] * ((mat[2][1] - means[2]) / Math.sqrt(vars[2] + 1e-8)) + shift[1],
            scale[2] * ((mat[2][2] - means[2]) / Math.sqrt(vars[2] + 1e-8)) + shift[2],
        ],
        [
            scale[0] * ((mat[3][0] - means[3]) / Math.sqrt(vars[3] + 1e-8)) + shift[0],
            scale[1] * ((mat[3][1] - means[3]) / Math.sqrt(vars[3] + 1e-8)) + shift[1],
            scale[2] * ((mat[3][2] - means[3]) / Math.sqrt(vars[3] + 1e-8)) + shift[2],
        ],
    ];
    const [result, _] = NeuralNet.layerNorm(mat, scale, shift);
...
test("completes a pass through the FFN sublayer", stats, () => {
    const X = Matrix.create(2, 2, -0.05, 0.05);
    const W1 = Matrix.create(2, 4, -0.05, 0.05);
    const b1 = Vector.create(4, -0.05, 0.05);
    const W2 = Matrix.create(4, 2, -0.05, 0.05);
    const b2 = Vector.create(2, -0.05, 0.05);

    const expected_a1 = [
        [
            Math.max(0, X[0][0]*W1[0][0] + X[0][1]*W1[1][0] + b1[0]),
            Math.max(0, X[0][0]*W1[0][1] + X[0][1]*W1[1][1] + b1[1]),
            Math.max(0, X[0][0]*W1[0][2] + X[0][1]*W1[1][2] + b1[2]),
            Math.max(0, X[0][0]*W1[0][3] + X[0][1]*W1[1][3] + b1[3]),
        ],
        [
            Math.max(0, X[1][0]*W1[0][0] + X[1][1]*W1[1][0] + b1[0]),
            Math.max(0, X[1][0]*W1[0][1] + X[1][1]*W1[1][1] + b1[1]),
            Math.max(0, X[1][0]*W1[0][2] + X[1][1]*W1[1][2] + b1[2]),
            Math.max(0, X[1][0]*W1[0][3] + X[1][1]*W1[1][3] + b1[3]),
        ],
    ];
    const expected = [
        [
            expected_a1[0][0]*W2[0][0] + expected_a1[0][1]*W2[1][0] + expected_a1[0][2]*W2[2][0] + expected_a1[0][3]*W2[3][0] + b2[0],
            expected_a1[0][0]*W2[0][1] + expected_a1[0][1]*W2[1][1] + expected_a1[0][2]*W2[2][1] + expected_a1[0][3]*W2[3][1] + b2[1],
        ],
        [
            expected_a1[1][0]*W2[0][0] + expected_a1[1][1]*W2[1][0] + expected_a1[1][2]*W2[2][0] + expected_a1[1][3]*W2[3][0] + b2[0],
            expected_a1[1][0]*W2[0][1] + expected_a1[1][1]*W2[1][1] + expected_a1[1][2]*W2[2][1] + expected_a1[1][3]*W2[3][1] + b2[1],
        ],
    ];
    const z1 = NeuralNet.transformerFFN(X, W1, b1);
    const a1 = z1.map(vector => Activations.relu(vector));
    const result = NeuralNet.transformerFFN(a1, W2, b2);
...
test("completes a backward pass for layer norm", stats, () => {
    ...
    const d_model = 4;
    const E = Matrix.create(vocab_size, d_model, -0.05, 0.05);
    const pos = NeuralNet.initSinusoidal(10, d_model);
    ...
    const scale = Vector.create(d_model, -0.05, 0.05);
    const shift = Vector.create(d_model, -0.05, 0.05);
    
    const embedding = NeuralNet.tokenize(sentence, vocab, E, pos);
    const z1 = NeuralNet.transformerFFN(embedding, W1, b1);
    const a1 = z1.map(vector => Activations.relu(vector));
    const z2 = NeuralNet.transformerFFN(a1, W2, b2);
    const z2_residual = Matrix.add(z2, embedding);
    const [
        norm_out,
        norm_cache,
    ] = NeuralNet.layerNorm(z2_residual, scale, shift);
    // norm_out[0] is used as a shortcut for the upstream CLS gradient
    const dL_dh = norm_out[0];
    const clsCache = norm_cache.CLS_normalized;
    const varCache = norm_cache.var;
    const [
        scale_grad_res,
        shift_grad_res,
        dL_dr_res
    ...
    const scale_grad_exp = Vector.elementwise(dL_dh, clsCache);
    const shift_grad_exp = dL_dh;
    const dL_dr_exp = new Float64Array(dL_dh.length);
    let dL_dh_mean = 0;
    let dL_dh_var = 0;
    ...
    for(let i = 0; i < dL_dh.length; i++) {
        dL_dr_exp[i] = 
            (scale[i] / Math.sqrt(varCache + 1e-8))
            * (dL_dh[i] - dL_dh_mean - dL_dh_var[i]);
    }

    return (
        dL_dr_exp[0] === dL_dr_res[0] &&
        dL_dr_exp[1] === dL_dr_res[1] &&
        dL_dr_exp[2] === dL_dr_res[2] &&
        dL_dr_exp[3] === dL_dr_res[3] &&
        shift_grad_exp[0] === shift_grad_res[0] &&
        shift_grad_exp[1] === shift_grad_res[1] &&
        shift_grad_exp[2] === shift_grad_res[2] &&
        shift_grad_exp[3] === shift_grad_res[3] &&
        scale_grad_exp[0] === scale_grad_res[0] &&
        scale_grad_exp[1] === scale_grad_res[1] &&
        scale_grad_exp[2] === scale_grad_res[2] &&
        scale_grad_exp[3] === scale_grad_res[3]
    );
});
test("completes a backward pass for FFN sublayer", stats, () => {
    const sentence = "hello there .";
    const vocab = {
        "hello": 0,
        "there": 1,
    ...
    const E = Matrix.create(vocab_size, d_model, -0.05, 0.05);
    const pos = NeuralNet.initSinusoidal(10, d_model);
    const W1 = Matrix.create(d_model, d_model, -0.05, 0.05);
    const b1 = Vector.create(d_model, -0.05, 0.05);
    const W2 = Matrix.create(d_model, d_model, -0.05, 0.05);
    const b2 = Vector.create(d_model, -0.05, 0.05);
    ...
    const a1 = z1.map(vector => Activations.relu(vector));
    const z2 = NeuralNet.transformerFFN(a1, W2, b2);
    // z2[0] is used as a shortcut for the upstream CLS gradient
    const [
        b2_grad_res,
        W2_grad_res,
        b1_grad_res,
        W1_grad_res,
        dL_dxFFN_res,
    ] = NeuralNet.FFNbackward(z2[0], a1[0], Activations.swishDerivative(a1[0], z1[0]), embedding[0], W1, W2);
    const b2_grad_exp = z2[0];
    const W2_grad_exp = Vector.outer(a1[0], z2[0]);
    const dL_dz1 = NeuralNet.backwardPass(
        Matrix.transpose(W2), z2[0], 
        Activations.swishDerivative(a1[0], z1[0])
    );
    const b1_grad_exp = dL_dz1;
    const W1_grad_exp = Vector.outer(embedding[0], dL_dz1);
    const dL_dxFFN_exp = Matrix.matrixVector(W1, dL_dz1);

    return (
        b2_grad_exp[0] === b2_grad_res[0] &&
        b2_grad_exp[1] === b2_grad_res[1] &&
        b2_grad_exp[2] === b2_grad_res[2] &&
        b2_grad_exp[3] === b2_grad_res[3] &&
        b1_grad_exp[0] === b1_grad_res[0] &&
        b1_grad_exp[1] === b1_grad_res[1] &&
        b1_grad_exp[2] === b1_grad_res[2] &&
        b1_grad_exp[3] === b1_grad_res[3] &&
        dL_dxFFN_exp[0] === dL_dxFFN_res[0] &&
        dL_dxFFN_exp[1] === dL_dxFFN_res[1] &&
        dL_dxFFN_exp[2] === dL_dxFFN_res[2] &&
        dL_dxFFN_exp[3] === dL_dxFFN_res[3] &&
        W1_grad_exp[0][0] === W1_grad_res[0][0] && 
        W1_grad_exp[0][1] === W1_grad_res[0][1] && 
        W1_grad_exp[0][2] === W1_grad_res[0][2] && 
        W1_grad_exp[0][3] === W1_grad_res[0][3] && 
        W1_grad_exp[1][0] === W1_grad_res[1][0] && 
        W1_grad_exp[1][1] === W1_grad_res[1][1] && 
        W1_grad_exp[1][2] === W1_grad_res[1][2] && 
        W1_grad_exp[1][3] === W1_grad_res[1][3] && 
        W1_grad_exp[2][0] === W1_grad_res[2][0] && 
        W1_grad_exp[2][1] === W1_grad_res[2][1] && 
        W1_grad_exp[2][2] === W1_grad_res[2][2] && 
        W1_grad_exp[2][3] === W1_grad_res[2][3] && 
        W1_grad_exp[3][0] === W1_grad_res[3][0] && 
        W1_grad_exp[3][1] === W1_grad_res[3][1] && 
        W1_grad_exp[3][2] === W1_grad_res[3][2] && 
        W1_grad_exp[3][3] === W1_grad_res[3][3] &&
        W2_grad_exp[0][0] === W2_grad_res[0][0] && 
        W2_grad_exp[0][1] === W2_grad_res[0][1] && 
        W2_grad_exp[0][2] === W2_grad_res[0][2] && 
        W2_grad_exp[0][3] === W2_grad_res[0][3] && 
        W2_grad_exp[1][0] === W2_grad_res[1][0] && 
        W2_grad_exp[1][1] === W2_grad_res[1][1] && 
        W2_grad_exp[1][2] === W2_grad_res[1][2] && 
        W2_grad_exp[1][3] === W2_grad_res[1][3] && 
        W2_grad_exp[2][0] === W2_grad_res[2][0] && 
        W2_grad_exp[2][1] === W2_grad_res[2][1] && 
        W2_grad_exp[2][2] === W2_grad_res[2][2] && 
        W2_grad_exp[2][3] === W2_grad_res[2][3] && 
        W2_grad_exp[3][0] === W2_grad_res[3][0] && 
        W2_grad_exp[3][1] === W2_grad_res[3][1] && 
        W2_grad_exp[3][2] === W2_grad_res[3][2] && 
        W2_grad_exp[3][3] === W2_grad_res[3][3]
    );
});
test("completes a backward pass for attention sublayer", stats, () => {
    const sentence = "hello there .";
    const vocab = {
        "hello": 0,
        "there": 1,
    ...
    const embeddings = NeuralNet.tokenize(sentence, vocab, E, pos);
    const [
        attention_out,
        attention_cache
    ...
    const dL_dr_exp = new Array(embeddings.length);
    for(let i = 0; i < dL_dr_exp.length; i++) {
        dL_dr_exp[i] = i === 0 ? attention_out[0] : new Float64Array(d_model);
    }

    const dL_dV = Matrix.multiply(
        Matrix.transpose(attention_cache.A),
        dL_dr_exp,
    );
    const dL_da = Matrix.multiply(
        dL_dr_exp,
        Matrix.transpose(attention_cache.V),
    );
    const dL_ds = new Array(dL_da.length);
    for(let i = 0; i < dL_da.length; i++) {
        const dot = Vector.dot(dL_da[i], attention_cache.A[i])
    ...
    const dL_dQKt = Matrix.divScalar(dL_ds, d_model);
    const dL_dQ = Matrix.multiply(dL_dQKt, attention_cache.K);
    const dL_dK = Matrix.multiply(Matrix.transpose(dL_dQKt), attention_cache.Q);

    const Wq_grad_exp = Matrix.multiply(Matrix.transpose(embeddings), dL_dQ);
    const Wk_grad_exp = Matrix.multiply(Matrix.transpose(embeddings), dL_dK);
    const Wv_grad_exp = Matrix.multiply(Matrix.transpose(embeddings), dL_dV);
    const dL_dxAttention_exp = Matrix.add(
        Matrix.multiply(
    ...
    return (
        Wq_grad_exp[0][0] === Wq_grad_res[0][0] &&
        Wq_grad_exp[0][1] === Wq_grad_res[0][1] &&
        Wq_grad_exp[1][0] === Wq_grad_res[1][0] &&
        Wq_grad_exp[1][1] === Wq_grad_res[1][1] &&
        Wk_grad_exp[0][0] === Wk_grad_res[0][0] &&
        Wk_grad_exp[0][1] === Wk_grad_res[0][1] &&
        Wk_grad_exp[1][0] === Wk_grad_res[1][0] &&
        Wk_grad_exp[1][1] === Wk_grad_res[1][1] &&
        Wv_grad_exp[0][0] === Wv_grad_res[0][0] &&
        Wv_grad_exp[0][1] === Wv_grad_res[0][1] &&
        Wv_grad_exp[1][0] === Wv_grad_res[1][0] &&
        Wv_grad_exp[1][1] === Wv_grad_res[1][1] &&
        dL_dr_exp[0][0] === dL_dr1_res[0][0] &&
        dL_dr_exp[0][1] === dL_dr1_res[0][1] &&
        dL_dr_exp[1][0] === dL_dr1_res[1][0] &&
        dL_dr_exp[1][1] === dL_dr1_res[1][1] &&
        dL_dr_exp[2][0] === dL_dr1_res[2][0] &&
        dL_dr_exp[2][1] === dL_dr1_res[2][1] &&
        dL_dr_exp[3][0] === dL_dr1_res[3][0] &&
        dL_dr_exp[3][1] === dL_dr1_res[3][1] &&
        dL_dxAttention_exp[0][0] === dL_dxAttention_res[0][0] &&
        dL_dxAttention_exp[0][1] === dL_dxAttention_res[0][1] &&
        dL_dxAttention_exp[1][0] === dL_dxAttention_res[1][0] &&
        dL_dxAttention_exp[1][1] === dL_dxAttention_res[1][1] &&
        dL_dxAttention_exp[2][0] === dL_dxAttention_res[2][0] &&
        dL_dxAttention_exp[2][1] === dL_dxAttention_res[2][1] &&
        dL_dxAttention_exp[3][0] === dL_dxAttention_res[3][0] &&
        dL_dxAttention_exp[3][1] === dL_dxAttention_res[3][1]
    );
});
test("successfully computes gradient for embedding matrix", stats, () => {
    const sentence = "hello there .";
    const vocab = {
        "hello": 0,
        "there": 1,
    ...
    const E = Matrix.create(vocab_size, d_model, -0.05, 0.05);
    const pos = NeuralNet.initSinusoidal(10, d_model);
    const mockUpstreamGradient = Matrix.create(4, d_model, -0.05, 0.05);

    const result = NeuralNet.embeddingBackward(mockUpstreamGradient, sentence, E, d_model, vocab);
    const expected = Array.from({ length: E.length }, () => new Float64Array(d_model));
    const _sentence = ("<CLS>" + " " + sentence).split(" ");
    for(let i = 0; i < _sentence.length; i++) {
        expected[vocab[`${_sentence[i]}`]] = Vector.add(
            expected[vocab[`${_sentence[i]}`]],
            mockUpstreamGradient[i]
        );
    }
    ...
    return (
        ...
        result[3][1] === expected[3][1]
    );
});
test("completes a backward pass for the classification head", stats, () => {
    const d_model = 4;
    const Wc = Matrix.create(3, d_model, -0.05, 0.05);
    const bc = Vector.create(3, -0.05, 0.05);
    const label = new Float64Array([0, 0, 1]);
    const mockCLS = new Float64Array([-0.2532, 0.12412, 0.058956]);
    const dL_dz = Cost.CCEDerivative_softmax(Activations.softmax(mockCLS), label);
    const bc_grad_exp = dL_dz;
    const Wc_grad_exp = Vector.outer(dL_dz, mockCLS);
    const dL_dh2_exp = Matrix.matrixVector(Matrix.transpose(Wc), dL_dz);
    const [
        bc_grad_res,
        Wc_grad_res,
        dL_dh2_res,
    ] = NeuralNet.classificationHeadBackward(dL_dz, mockCLS, Wc);

    return (
        bc_grad_exp[0] === bc_grad_res[0] &&
        bc_grad_exp[1] === bc_grad_res[1] &&
        bc_grad_exp[2] === bc_grad_res[2] &&
        dL_dh2_exp[0] === dL_dh2_res[0] &&
        dL_dh2_exp[1] === dL_dh2_res[1] &&
        dL_dh2_exp[2] === dL_dh2_res[2] &&
        Wc_grad_exp[0][0] === Wc_grad_res[0][0] &&
        Wc_grad_exp[0][1] === Wc_grad_res[0][1] &&
        Wc_grad_exp[0][2] === Wc_grad_res[0][2] &&
        Wc_grad_exp[0][3] === Wc_grad_res[0][3] &&
        Wc_grad_exp[1][0] === Wc_grad_res[1][0] &&
        Wc_grad_exp[1][1] === Wc_grad_res[1][1] &&
        Wc_grad_exp[1][2] === Wc_grad_res[1][2] &&
        Wc_grad_exp[1][3] === Wc_grad_res[1][3] &&
        Wc_grad_exp[2][0] === Wc_grad_res[2][0] &&
        Wc_grad_exp[2][1] === Wc_grad_res[2][1] &&
        Wc_grad_exp[2][2] === Wc_grad_res[2][2] &&
        Wc_grad_exp[2][3] === Wc_grad_res[2][3]
    );
});
test("successfully trains a model to analyze sentiment of sentences", stats, () => {
    const text = [
    ...
    for(let i = 0; i < EPOCHS; i++) {
        for(let x = 0; x < text.length; x++) {
            ...
            const [
                ...
                dL_dr2,
                FFN_a1[0],
                Activations.swishDerivative(FFN_a1[0], FFN_z1[0]),
    ...
    return (
        ...
        results[2].indexOf(Math.max(...results[2])) === labels[2].indexOf(1) &&
        results[3].indexOf(Math.max(...results[3])) === labels[3].indexOf(1) &&
        results[4].indexOf(Math.max(...results[4])) === labels[4].indexOf(1) &&
        results[5].indexOf(Math.max(...results[5])) === labels[5].indexOf(1)
    );
});
