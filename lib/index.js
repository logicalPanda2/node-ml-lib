...
export class NeuralNet {
    ...
    static convForwardMatSrc(src, kernels, b) {
    ...
    static convForwardTenSrc(src, kernels, b) {
        const output = new Array(kernels.length);

        for(let i = 0; i < kernels.length; i++) {
            let featMap = Array.from(
                { kernels: kernels[0].length },
                () => new Float64Array(kernels[0][0].length)
            );
            for(let j = 0; j < src.length; j++) {
                const part = Matrix.convolve(src[j], kernels[i]);
