...
export class NeuralNet {
    ...
    static tokenize(spaceSeparatedInput, vocabulary, embeddingMat) {
        const sentence = spaceSeparatedInput.split(" ");
        const tokenized = new Array(sentence.length);
        for(let i = 0; i < sentence.length; i++) {
            tokenized[i] = embeddingMat[vocabulary[`${sentence[i]}`]];
        }
    ...
    static attention(embeddings, Wq, Wk, Wv, d_k) {
        ...
        for(let i = 0; i < QK_T.length; i++) {
            QK_T[i] = Activations.softmax(QK_T[i]);
        }
        cache.A = QK_T;

        return [
            Matrix.multiply(QK_T, V), 
            cache,
        ];
    }
    static layerNorm(residual, scale, shift) {
        ...
        for(let i = 0; i < residual.length; i++) {
            ...
            for(let j = 0; j < residual[0].length; j++) {
                normalized[i][j] = scale[j] * ((residual[i][j] - means_var[i][0]) / Math.sqrt(means_var[i][1] + 1e-8)) + shift[j];
            }
        }
