...
export class Matrix {
    ...
    static fullConvolve(src, kernel) {
        Shared.validateStructureMatSingle(src);
        Shared.validateStructureMatSingle(kernel);
...
export class NeuralNet {
    ...
    static convForward(src, kernels, b) {
        Shared.validateStructure3DTensorSingle(src);
        Shared.validateStructureTensorSingle(kernels);
    ...
    static convBackwardPrev(W_l, delta_L) {
        ...
        for(let i = 0; i < W_l[0].length; i++) {
            ...
            for(let j = 0; j < delta_L.length; j++) {
                // if an error is thrown anywhere from this function, the indexing is faulty
                const part = Matrix.fullConvolve(delta_L[j], Matrix.flip(W_l[j][i]));
                featMap = Matrix.add(featMap, part);
            }
            delta_l[i] = featMap;
        }
    ...
    static convBackwardGAP(delta_L, W, channels, rows, cols, activationGrad) {
        Shared.validateStructure3DTensorSingle(activationGrad);
        
        const delta_l = Matrix.matrixVector(Matrix.transpose(W), delta_L);
        const dL_dz = Array.from(
            { length: channels }, 
            () => Array.from(
                { length: rows },
        ...
        for(let i = 0; i < delta_l.length; i++) {
            const grad = delta_l[i] / (cols * rows);
            for(let j = 0; j < rows; j++) {
                for(let k = 0; k < cols; k++) {
                    dL_dz[i][j][k] = grad * activationGrad[i][j][k];
                }
            }
        }
